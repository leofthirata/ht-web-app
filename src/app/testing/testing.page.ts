...
export class TestingPage implements OnInit {
  ...
  public async getKey() {
    ...
    const onOpen = await socket.open(`ws://${this.ip}/get_key`, this.m_auth.getPrivKey());
    console.log('open');
  ...
  public async setSecret() {
    ...
    const socket = new WebSocketService(this.m_state);
    const onOpen = await socket.open(`ws://${this.ip}/set_secret`, this.m_auth.getPrivKey());
  ...
  public async register() {
    ...
    const dev = await createDevice(this.m_secret, accessToken, env, 'Device1', this.ip, '123123', '321321', 'PADOTEC', 'one', forge.pki.publicKeyToPem(this.m_devicePublicKey));
  ...
  public async setTicket() {
    ...
    const socket = new WebSocketService(this.m_state);
    const onOpen = await socket.open(`ws://${this.ip}/set_ticket`, this.m_auth.getPrivKey());
